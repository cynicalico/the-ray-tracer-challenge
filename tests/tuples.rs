...
fn test_a_tuple_with_w_eq_1_is_a_point() {
    let a = trtc::Tuple4::from_array([4.3, -4.2, 3.1, 1.0]);
...
fn test_a_tuple_with_w_eq_0_is_a_vector() {
    let a = trtc::Tuple4::from_array([4.3, -4.2, 3.1, 0.0]);
...
fn test_point_creates_tuples_with_w_eq_1() {
    ...
    assert_eq!(a, trtc::Tuple4::from_array([4.0, -4.0, 3.0, 1.0]));
}
...
fn test_vector_creates_tuples_with_w_eq_0() {
    ...
    assert_eq!(a, trtc::Tuple4::from_array([4.0, -4.0, 3.0, 0.0]));
}

#[test]
fn test_adding_two_tuples() {
    let a1 = trtc::Tuple4::from_array([3.0, -2.0, 5.0, 1.0]);
    let a2 = trtc::Tuple4::from_array([-2.0, 3.0, 1.0, 0.0]);

    assert_eq!(a1 + a2, trtc::Tuple4::from_array([1.0, 1.0, 6.0, 1.0]));
}
...
fn test_negating_a_tuple() {
    let a = trtc::Tuple4::from_array([1.0, -2.0, 3.0, -4.0]);

    assert_eq!(-a, trtc::Tuple4::from_array([-1.0, 2.0, -3.0, 4.0]));
}
...
fn test_multiplying_a_tuple_by_a_scalar() {
    ...
    assert_eq!(a * 3.5, trtc::Tuple4::from_array([3.5, -7.0, 10.5, -14.0]));
}
...
fn test_multiplying_a_tuple_by_a_fraction() {
    ...
    assert_eq!(a * 0.5, trtc::Tuple4::from_array([0.5, -1.0, 1.5, -2.0]));
}
...
fn test_dividing_a_tuple_by_a_scalar() {
    ...
    assert_eq!(a / 2.0, trtc::Tuple4::from_array([0.5, -1.0, 1.5, -2.0]));
}
...
fn test_normalizing_vector_4_0_0_gives_1_0_0() {
    ...
    assert_eq!(v.normalize(), trtc::vector(1.0, 0.0, 0.0));
}
...
fn test_the_cross_product_of_two_vectors() {
    ...
    assert_eq!(a.cross(&b), trtc::vector(-1.0, 2.0, -1.0));
    assert_eq!(b.cross(&a), trtc::vector(1.0, -2.0, 1.0));
}
